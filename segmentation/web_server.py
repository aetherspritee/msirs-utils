...
def upload_file():
    ...
    folder = home_dir+"/segmentation/static/"
    for filename in os.listdir(folder):
        file_path = os.path.join(folder, filename)
        try:
            if (os.path.isfile(file_path) or os.path.islink(file_path)) and not re.findall("home", file_path):
                os.unlink(file_path)
    ...
    if uploaded_file.filename != '':
        ...
        server = Server()
        server.connection.put(path, f"/home/{server.username}/query/"+str(uploaded_file.filename))
        ...
        # TODO: dddddddd
        # stdin, stdout, stderr = server.ssh.exec_command("source ~/codebase-v1/venv/bin/activate && python3 ~/segmentation/first_full_pipeline.py")
        ...
        stdin, stdout, stderr = server.ssh.exec_command("ls ~/server-test | grep retrieval")
        output = stdout.readlines()
        for item in output:
            item = item[:-1]
            print(item)
            server.connection.get(f"/home/{server.username}/server-test/{item}", home_dir+f"/segmentation/static/{item}")
...
@app.route("/results")
def results():
    res_path = home_dir+"/segmentation/static/"
...
class Server():
    def __init__(self):
        ...
        self.ssh.connect(self.hostname, username=self.username, password=password, port=port)
