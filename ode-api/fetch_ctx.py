...
def download_images(
    ...
    query_results = query_processor.query_files_urls(
        target,
        mission,
        instrument,
        product_type,
    ...
    SAVE_DIR_PREFIX = "database/"
    CHUNK_SIZE = 1024

    # Default Settings to Skip all black images
    SKIP_BLACK_IMAGES = (
        True  # Set to False to retain all images that contain black pixels
    )

    # Default Settings to Align and Crop images with black margins
    MAX_BORDER_SIZE = 200  # Border to be checked around the image
    SAFETY_MARGIN = (
        0  # Removes extra pixels from the sides to make sure no black remains
    )
    TOLERANCE = 10  # A gray value is more likely to be considered black when you increase the tolerance
    ALIGN_AND_CROP_THRESHOLDS = (
        MAX_BORDER_SIZE,
        SAFETY_MARGIN,
        TOLERANCE,
    )  # Set to None to prevent alignment and cropping

    query_result_processor = QueryResultProcessor()
    should_continue = query_result_processor.download2(query_results, bin_type)
    if should_continue:
...
if __name__ == "__main__":
    ...
    western_lon = 15.18  # Western longitude to look for the data, from 0 to 360
    eastern_lon = 15.31  # Eastern longitude to look for the data, from 0 to 360
    min_lat = -56.9  # Minimal latitude to look for the data, from -90 to 90
    max_lat = -56.66  # Maximal latitude to look for the data, from -90 to 90
    ...
    max_ob_time = (
        ""  # Maximal observation time in (even partial) UTC format, e.g., '2017-03-01'
    )
    ...
    result_offset_number = (
        0  # Offset the return products, to go beyond the limit of 100 returned products
    )
