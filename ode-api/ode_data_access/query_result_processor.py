...
class QueryResultProcessor:
    ...
    def download(self, query_results, bin_types=None, product_types=None):
        ...
        print(
            "Required Product Names matching the given bin type =",
    ...
    def download2(self, query_results, bin_types=None, product_types=None):
        self.find_required_products(query_results, bin_types)
        self.find_required_product_image_urls(query_results, product_types)
        print(
            "Required Product Names matching the given bin type =",
    ...
    def download3(self, query_results, bin_types=None, product_types=None):
        self.find_required_products(query_results, bin_types)
        self.find_required_product_image_urls(query_results, product_types)
        print(
            "Required Product Names matching the given bin type =",
        ...
        print("----")
        image_names = [i for i in self.required_products]
        return image_names
    ...
    def find_required_products(self, query_results, bin_types=None):
        for query_result in query_results.keys():
            ...
            if product_type == "PRODUCT LABEL FILE":
                ...
                if bin_types is None or self.get_bin_type(binning) == bin_types:
    ...
    def find_required_product_image_urls(self, query_results, product_types):
        ...
        for query_result in query_results.keys():
            product_name, product_type = query_results[query_result]
            #print(f"{product_type in product_types = }")
            #print(f"{product_name in self.required_products = }")
            if (
                product_type in product_types
            ) and product_name in self.required_products:
